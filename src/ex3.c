...
void p2p_i_transmit_A(int p, int q, Matrix *A, int i, int l) {
  ...
  if (me == Ail->owner /* I own A[i,l]*/ ) {
    ...
    for (int k = 0; k < q; k++) {
    	...
    	if (node != me) {
    		MPI_Issend (Ail->c , b*b , MPI_FLOAT, node, 0, MPI_COMM_WORLD, &(Ail->request));
    	}
    }
  } else if (Ail->row == my_row /* A[i,l] is stored on my row */) {
    Ail->c = calloc(b*b,sizeof(float));
    // MPI_Irecv Ail
    MPI_Irecv(Ail->c , b*b , MPI_FLOAT, Ail->owner, 0, MPI_COMM_WORLD, &(Ail->request));
  }
  /* end TODO */
}
...
void p2p_i_transmit_B(int p, int q, Matrix *B, int l, int j) {
  ...
  if (me == Blj->owner /* I own B[l,j]*/ ) {
    ...
    for (int k = 0; k < p; k++) {
    	...
    	if (node != me) {
    		MPI_Issend (Blj->c , b*b , MPI_FLOAT, node, 0, MPI_COMM_WORLD, &(Blj->request));
    	}
    }
  } else if (Blj->col == my_col /* B[l,j] is stored on my column */) {
    Blj->c = calloc(b*b,sizeof(float));
    // MPI_Irecv B[l,j]
    MPI_Irecv(Blj->c , b*b , MPI_FLOAT, my_col, 0, MPI_COMM_WORLD, &(Blj->request));
  }
  /* end TODO */
}
...
void p2p_i_wait_AB(int p, int q, Matrix *A, Matrix* B, Matrix* C, int l) {
  ...
  for (i = 0; i < A->mb ; i++) {
    Ail = &(A->blocks[i][l]);
    if (Ail->row == my_row/* I need A[i,l] for my computation */) {
      // MPI_Wait Ail
  ...
  for (j =0; j < B->nb; j++) {
    Blj = &(B->blocks[l][j]);
    if (Blj->col == my_col/* I need B[l,j] for my computation */) {
      // MPI_Wait Blj
